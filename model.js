  ...
  function fairValue(listings, sold) {
    ...
    const feat = r => {
      ...
      const f = [1, la, la * la, roomsOf(r), (r.a && roomsOf(r) ? r.a / roomsOf(r) / 40 : 0),
        ...
        // model ignored it — and it was confounded with etage, which it did use:
        // only 33 % of stuelejligheder have an altan against 66 % on 1. sal, so
        // "stue is cheaper" and "stue rarely has an altan" were landing on the
        // same coefficient. Its own coefficient is a sensible +2,4 %.
        //
        // Elevator was tried here too and predicted slightly better (−0,14pp),
        // but its coefficient came out NEGATIVE (−3,6 %): after kommune, byggeår
        // and comps are controlled for, "has a lift" mostly marks post-war blocks,
        // which are cheaper per m² — it proxies building type, not the amenity.
        // That is harmless for scoring listings but wrong as a lever on the
        // lookup form, where toggling "elevator: ja" then *lowered* the value.
        // A hedonic coefficient is not a ceteris-paribus effect, so it is left
        // out rather than shown to users as one.
        (r.near ? 1 : 0), (r.t === 'villa' ? 1 : 0), (r.balc ? 1 : 0)];
